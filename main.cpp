...
ifstream file(fichier.c_str(), ios::in);  // on ouvre en lecture
  ...
  if(file){
    ...
    while(getline(file, contenu)){
      for(int j=0; j<m; j++){
	if(contenu[j]==' ')
	  floor.setBoard(i,j,space);
	if(contenu[j]=='.')
	  floor.setBoard(i,j);
	else if(contenu[j]=='X')
	  floor.setBoard(i,j,source);
	else if(contenu[j]=='#')
	  floor.setBoard(i,j,wall);
	else if(contenu[j]=='U')
	  floor.setBoard(i,j,s_up);
	else if(contenu[j]=='D')
	  floor.setBoard(i,j,s_down);
	else if (contenu[j]=='c'){
	  ...
	  floor.setBoard(i,j,*warrior2);
	  warrior2->setFloor(&floor);
	}else if (contenu[j]=='<'|| contenu[j]=='>' ||contenu[j]=='^'||contenu[j]=='v'){
	  Player * player = new Player(&floor,i,j,"Ida");
	  floor.setBoard(i,j,*player);
	}
      }
      i++;
